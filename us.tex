...
except that all the \gls{TRU} is reprocessed from \gls{LWR} \gls{UNF}
to fabricate fuel salt, instead of \gls{MOX}. Also, the depleted uranium from
the enrichment plant is stored in a storage facility to be used for fabricating
\gls{MSR} fuel salt and as a fertile stream for \gls{MSR} facilities. 
...
\begin{table}[h]
	...
	\begin{tabular}{lr}
		...
		\hline
		\multicolumn{2}{c}{SERPENT Parameters}
		Num. neutrons per generation & 8000 \\
		Num. active generation & 150\\
		Num. inactive generation & 50 \\
		Burnup calc. mode & CRAM \\
		Power density & $32.18e-3$ kW/g \\ 
		Depletion step & 30 days\\
		
		\hline
		\multicolumn{2}{c}{Saltproc Parameters}
		Lifetime [y]  & 30 \\
		Total timesteps & 365 \\
